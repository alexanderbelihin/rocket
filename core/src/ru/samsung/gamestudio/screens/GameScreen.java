...
public class GameScreen extends ScreenAdapter {
    ...
    // Флаг, чтобы сохранить рекорд только один раз
    ...
    public void show() {
        restartGame();
        recordSaved = false;  // Сбрасываем флаг при показе экрана
    }
    ...
    public void render(float delta) {
        ...
        if (gameSession.state == GameState.PLAYING) {
            ...
            if (!shipObject.isAlive() && !recordSaved) {
                gameSession.endGame();
                recordsListView.setRecords(MemoryManager.loadRecordsTable());

                // Сохраняем лучший результат сбитых предметов
                MemoryManager.saveBestDestroyed(trashDestroyed);
                recordSaved = true;  // Чтобы не сохранять несколько раз

                // Для отладки (можно убрать потом)
                System.out.println("Game over! Destroyed: " + trashDestroyed);
            }
        }
    ...
    private void draw() {
        ...
        myGdxGame.commonWhiteFont.draw(myGdxGame.batch, "Destroyed: " + trashDestroyed, 20, 1185);
