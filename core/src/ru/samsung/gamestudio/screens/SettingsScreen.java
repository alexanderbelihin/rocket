...
public class SettingsScreen extends ScreenAdapter {
    ...
    // НОВОЕ: для сообщения об очистке
    ...
    public SettingsScreen(MyGdxGame myGdxGame) {
        ...
        confirmMessageView = new TextView(myGdxGame.commonWhiteFont, 173, 540, "");
    }
    ...
    public void render(float delta) {
        ...
        // НОВОЕ: обновление таймера сообщения
        if (messageTimer > 0) {
            messageTimer -= delta;
        }
        ...
        // НОВОЕ: рисуем сообщение, если таймер активен
        if (messageTimer > 0) {
            confirmMessageView.draw(myGdxGame.batch);
        }
    ...
    void handleInput() {
        if (Gdx.input.justTouched()) {
            ...
            if (clearSettingView.isHit(myGdxGame.touch.x, myGdxGame.touch.y)) {
                ...
                // НОВОЕ: показываем сообщение вместо изменения текста кнопки
                confirmMessageView.setText("Records cleared!");
                messageTimer = 2f;  // показывать 2 секунды
            }
